    ...
    <article className="pattern-detail shell">
      ...
      <div className="detail-layout">
        ...
        <aside className="detail-panel">
          ...
          <section className="download-panel"><h2>{isPaid ? "Premium access" : "Download pattern"}</h2>{isPaid ? <PremiumPurchase slug={pattern.slug} title={pattern.title} category={pattern.categorySlug} /> : <><p>{liveDownloads ? "Download the free chart image." : "This free pattern will be added with the final artwork."}</p><div className="download-actions">{liveDownloads ? <MosaicDownloadButton href={pattern.downloadImage} filename={`${pattern.slug}-fusemosaic.jpg`} pattern={{ slug: pattern.slug, title: pattern.title, category: pattern.categorySlug }} /> : <span className="download-button" aria-disabled="true">Download unavailable</span>}</div><p className={styles.personalUse}>Free for personal craft use.</p></>}</section>
