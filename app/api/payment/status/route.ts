...
export async function GET(request: Request) {
  ...
  if ((!paymentId && !slug) || (paymentId && slug) || (paymentId && paymentId.length > 200) || (slug && (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug) || slug.length > 160))) {
    return Response.json({ error: "A valid payment ID or pattern slug is required" }, { status: 400 });
  }
